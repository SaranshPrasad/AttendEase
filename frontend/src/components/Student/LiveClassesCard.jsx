...
export default function LiveClassesCard({
  ...
  const fetchAllStats = async () => {
    try {
      const requests = sessions.map((session) =>
        axios
          .get(`${import.meta.env.VITE_API_URL}/attendance/session/${session._id}/stats`)
  ...
  return (
    <Card className="bg-white/80 backdrop-blur-sm border border-gray-100 shadow-lg h-full">
      <CardHeader>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="max-h-[70vh] overflow-y-auto scrollbar-hide space-y-4 pr-1">
